...
/**
 * \file    test/phpunit/DoliLogisticFunctionalTest.php
...
/**
 * Class DoliLogisticFunctionalTest
 *
 ...
 *
 * @package Testdolilogistic
 */
class DoliLogisticFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modDoliLogistic')->click();
		} else {
			// Disable the module
			$this->byHref('modDoliLogistic')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/dolilogistic/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('dolilogistic/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/dolilogistic/admin/about.php');
		$this->authenticate();
		return $this->assertContains('dolilogistic/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('dolilogisticwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modDoliLogistic_DoliLogisticTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modDoliLogistic_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
